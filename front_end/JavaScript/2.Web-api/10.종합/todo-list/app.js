...
function makeNewToDoNode(newTodo) {
    ...
    //클래스 이름을 두개 이상 add할 때는 각각 지정해야 합니다.
...
function insertToDoData() {
    ...
    //공백이 들어갈 가능성이 있기 때문에 공백을 제거하고 비교. trim() = 공백 제거
...
//data-id값으로 배열을 탐색하여 일치하는 객체가 들어있는 인덱스 반환하는 함수 정의
...
function changeCheckState($label) {
    ...
    //할 일이 완료된 노드의 클래스 이름을 추가(디자인 주기위해)
    //checked를 상황에 따라서 추가하거나 삭제할 수 있어야 하기 때문에 toggle함수로 처리해 주도록 하겠습니다.
    $label.classList.toggle('checked');

    //dataId를 기반으로 배열을 탐색하여 data-id와 일치하는 
    //id프로퍼티를 가진 객체의 인덱스를 얻어올 예정.
    const dataId = +$label.parentNode.dataset.id;
    const index = findIndexByDataId(dataId);

    todos[index].done = !todos[index].done; //논리반전을 이용해서 기존에 있던것을 반대로 전환 false 였다면 true로 전환
    console.log(todos);
}
...
function removeToDoData($delTarget) {
    ...
    //removeChild 진행 전에 애니메이션 발동 및 배열 내부 객체가 삭제가
    //진행 될 수 있도록 시간을 지연.
    setTimeout(() => {
...
function enterModifyMode($modSpan) {
    ...
    //span.text를 input 태그로 교체 (span 태그를 삭제하고 input태그를 추가)
...
(function() {
    ...
    $addBtn.addEventListener('click', e => {
        ...
        //from태그 안의 button은 type을 명시하지 않으면 자동 submit이 실행됩니다.
    ...
    $todoList.addEventListener('change', e => {
        ...
        changeCheckState(e.target.parentNode); //label를 함수의 매개값으로 전달.
    });
    ...
    $todoList.addEventListener('click', e => {
        ...
        removeToDoDate(e.target.parentNode.parentNode);
    });
