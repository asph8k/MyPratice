...
function makeNumberIcons() {
    ...
    const $frag = document.createDocumentFragment(); //가상 DOM 생성

    for(let i=gameData.min; i<=gameData.max; i++) {
        const $icon = document.createElement('div');
        $icon.classList.add('icon'); //icon클래스를 생성
        $icon.textContent = i;
...
function clearNumberIcons($numbers) {
    for(let $icon of [...$numbers.children]) { //향상 포문에서는 요소를 제거하는 행위는 안하는 것이 좋음.
...
function processCorrect($target) {
    //축하메세지 박스를 나타내개 하는 코드
    const $finish = document.getElementById('finish');
    //finish박스는 평소에는 숨겨져 있음. 이제는 드러나도 된다는 신호.
    $finish.classList.add('show'); //shwo 클래스를 추가.
...
function checkAnswer($numbers, $target) {
    //객체에서 정답과 사용자의 선택값을 동시에 가져오기
    const {secret, answer} = gameData;
    console.log(secret);
    const $begin = document.getElementById('begin');
    const $end = document.getElementById('end');

    //정답을 맞췄을 시 정답처리 하는 함수 호출(processCorrect)
...
//메인 역할을 할 핵심 실행 로직 즉시 실행 함수 (main 역할)
(function() {
    ...
    $numbers.addEventListener('click', e => {
        ...
        gameData.answer = +e.target.textContent; //+를 붙여서 정수로 변환해서 넣어준다.
        console.log(`${e.target.textContent}클릭됨!`); //값부터 먼저 클릭되는지 확인하기!
