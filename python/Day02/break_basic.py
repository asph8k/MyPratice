...
반복 횟수가 끝날 때 까지 멈추지 않고 반복을 실행합니다.
- 하지만 중간에 어떠한 이유로 반복을 중지해야 한다거나 현재 반복을 건너 뛰어야 할 경우에
탈출문을 사용합니다.

*break
- break는 현재 반복문을 즉시 종료시키고 반복문을 탈출합니다.
- 일반적으로는 특정 조건하에서 반복문을 종료시키기 때문에 대부분은 if문과 함께 사용합니다.
'''
...
'''
* 무한 루프
- 무한 루프는 반복 횟수를 정하지 않고 무한하게 반복을 실행하는 구조입니다.
- 사전에 정확한 반복 횟수를 알 수 없을 때 구현합니다.
탈출문을 통해 종료할 수 있도록 설계하면 됩니다.
- 파이썬의 무한 루프는 while문으로만 작성이 가능하며 for문으로는 구현이 불가능합니다.
'''
