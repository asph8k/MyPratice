...
soup = BeautifulSoup(src, 'html.parser') # 파이썬은 new를 쓰지않고 바로 생성자를 작성

'''
- 뷰티풀수프를 사용하여 수집하고 싶은 데이터가 들어있는 태그를 부분 추출할 수 있습니다.
- find_all()메소드 : 인수값으로 추출하고자 하는 태그의 이름을 적으면 해당 태그만 전부 추출하여
리스트에 담아 대입합니다.
'''
...
# text는 태그를 제외한 사용자가 실제로 브라우저에서 확인 가능한 텍스트 만을 추출하여
# 문자열 형태로 반환합니다.
