...
file_path = f'C:/Users/User/Desktop/java_web_RKP/python/crawling/알라딘 베스트셀러 1~400위(엑셀)_{d.year}_{d.month}_{d.day}.xlsx'
...
# 액셀 처리 선언
# workbook 객체를 생성하여 액셀 파일을 하나 생성(매개값으로 저장될 경로를 지정.)
...
# 브라우저 설정 - headless 모드
driver = webdriver.Chrome('C:/Users/User/Desktop/java_web_RKP/python/chromedriver_win32/chromedriver.exe', options=chrome_option)
...
while True:
    ...
    for div_ss_book_box in div_ss_book_box_list:
        ...
        # 책 상세 정보 페이지 링크
        # title이란 변수에 a태크를 이미 지목해 놓은 상태.
        # title -> a태크의 요소 전부를 가지고 있는 상태
    ...
    # 다음 페이지(탭)로 전환
    cur_page_num += 1
    driver.find_element(By.XPATH, f'//*[@id="newbg_body"]/div[3]/ul/li[{cur_page_num}]/a')
    del(soup)
    time.sleep(3)
