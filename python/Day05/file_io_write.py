...
'''
# 파일 입출력은 외부 데이터로 접근하기 때문에 예외처리를 통해 안전하게 작성합니다.
try:
    # 파일 저장 기능 (write, append)
    file_path = r'C:\Users\user\Desktop\java_web_RKP\front_end\python\test\test2.txt'

    # 파일 입출력을 실행하는 내장 함수 open()
    # 반환값으로 파일 입출력을 담당하는 객체가 리턴됩니다.
    # 첫번째 인수로 파일 경로를, 두번째 인수로 모드를 설정.
    f = open(file_path, 'a') # 문자열 형태로 모드 선언
