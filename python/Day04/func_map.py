'''
* 내장함수 map()
- map()은 첫번째 인수로 함수를 지정하고 두번째 인수로 리스트를 지정하면 해당 리스트 내부
요소값들을 일괄적으로 첫번째 인수로 지정한 함수에 인수로 전달합니다.
'''
...
# li 안에 있는 데이터들을 triple_squared 함수로 전달해서 값을 변환하기 위해 사용.
# map 함수가 리턴하는 객체를 list로 변환해서 확인.
result = map(triple_squared, li)
print(result)