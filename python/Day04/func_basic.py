...
- 함수는 한번 정의해두면 지정된 함수 이름을 통해 언제든지 해당 코드 블록을
실행할 수 있습니다.
...
def calc_sum(x): # 매개변수 : 함수가 필요로 하는 값을 받아주는 역할
    sum = 0
    for n in range(1, x+1):
        sum += n
    return sum # return : 함수를 부른쪽으로 값을 반환
...
- 인수의 개수는 제한이 없어서 많은 값을 함수에 전달할수도 있고 하나도 전달하지 않을 수 있습니다.
- 파이썬의 경우에는 데이터 타입을 작성하지 않기 때문에 이 함수를 처음 사용하는 사람도
인수 이름만 보고 무슨 값을 전달해야 할지 의미를 알기 쉽게 지정하는 것이 좋습니다.  
'''
...
print('3~7까지의 총 누적합:', calc_rangesum(3, 7))
'''
def calc_rangesum(start, end):
    if start > end:
        start, end = end, start
...
print(add(add(5,7), add(9,8))) # add(12, 17) 과 같은 결과를 얻어내는 것과 같다.
...
def multi(n1, n2): # 자바의 void와 같은 리턴이 없음.
    result = n1 * n2
