'''
* 튜플 (tuple)
- 튜플은 값의 집합이라는 측면에서는 리스트와 유사하지만 값을 한번 저장한 후에는 내부 요소의
편집이 불가능합니다.
- 튜플은 상수(고정) 리스트라고도 부르며 ()를 사용하여 표현합니다.
- 튜플도 문자열과 리스트와 마찬가지로 시퀀스 자료형 입니다.(인덱스 있음)
'''
...
for p in points:
    sum += p
print(f'총점: {sum}점, 평균: {sum / len(points):0.1f}')

# 튜플을 만들때는 ()를 생략할수 있습니다.
print('-'*50)
tu = 1,3,5,7,9
print(type(tu))
...
# tu.append(10) (X)
# del(tu[0]) (X)
# tuple이 지원하는 메소드는 index, count 뿐 입니다.
