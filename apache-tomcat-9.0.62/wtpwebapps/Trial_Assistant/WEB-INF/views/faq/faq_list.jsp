		...
		<section class="py-3 total-sec ">
			<div class="table-box col-lg second-section">
				<div class="more-view title-box" id="notice_title">
					...
					<form action="<c:url value='/faqans/list'/>">
						<div class="announcement-search dropup" id="total">
							<select class="form-control search-select" name="condition"
								...
								<option value="writer"
									${pcv.paging.condition == 'writer' ? 'selected' : ''}>내용</option>
					...
					<tbody style="border-top: 3px solid #e9ecef">
						...
						<c:forEach var="faq" items="${faqList }">
							<tr>
								<td>${faq.faqNum}</td>
								<td><a
									href="<c:url value='/faqans/content/${faq.faqNum }' />"
									class="link-a-style">${faq.faqTitle }</a></td>
								<td>${faq.JOINID }</td>
								<td><fmt:formatDate value="${faq.faqDate}"
					...
					<div class="text-center search-select">
						<ul id="pagination" class="pagination pagination-sm">
							...
							<c:forEach var="page" begin="${pcv.beginPage }"
								...
								<!-- 페이지 구성  -->
								<li class="${pcv.paging.pageNum == page ? 'active' : '' }"
									class="pageBtn"><a href="#" data-pagenum="${page}"
							...
							<c:if test="${pcv.next}">
								<li class="pageBtn"><a href="#"
									data-pagenum="${pcv.endPage+1 }">다음</a></li>
							</c:if>
							<li>
	...
	$(function() {
		$('#inputGroupFileAddon04').click(function() {
			...
			if (confirm('공지사항을 등록하시겠습니까? ')) {
				location.href = '<c:url value = "/faqans/write" />';
			} else {
				return 
			...
			<div class="row">
				...
				<div class="col-lg-3">
					...
					<ul class="list-unstyled templatemo-accordion">
						<li class="pb-3"><a
							class="collapsed d-flex justify-content-between h3 text-decoration-none"
							href="<c:url value='/faqans/list'/>"> FAQ List </a></li>
