                ...
                <div>
                    ...
                    <form name="delGhForm" action="<c:url value='/app/appNormalUnq'/>" method="post">
                    	...
                    	<div class="input-group mb-3">
									...
									<input type="text" class="form-control" aria-label="Text input with segmented dropdown button" value="${AppGhDet.anmTitle}" readonly>
									<span class="input-group-text">공고 차수</span>
								    <input type="text" class="form-control" aria-label="Text input with segmented dropdown button" name="JoinId" value="${AppGhDet.anmDegree}" readonly> 
                        ...
                        <div class="input-group mb-3">
                            ...
                            <input type="text" id = "ghId" class="form-control" aria-label="Text input with segmented dropdown button" value="${AppGhDet.joinId}" readonly>
...
<%--                             <span class="input-group-text">경력 파일</span>
 ...
 --%>
						...
						<div class="input-group mb-3">
							...
							<input type="text" id = "unqualify" class="form-control" aria-label="Text input with segmented dropdown button" value="${AppGhDet.ghUnqualify}" readonly> 
							<span class="input-group-text">승인여부</span> 
							<input type="text" id = "qualify" class="form-control" aria-label="Text input with segmented dropdown button" value="${AppGhDet.ghQualify}" readonly> 
									
							<button class="btn btn-outline-secondary" type="button" id="inputGroupFileAddon05">다운로드</button> 									
		                    <input type="text" class="form-control" id="inputGroupFile04" aria-describedby="inputGroupFileAddon04"  value="${AppGhDet.ghTotalFileReal }" readonly>
                ...
                <div class="notice-regi-btn">
					...
					<button class="btn btn-outline-secondary" type="button" id="unqualifyBtn">부적격자처리</button>
                   	<button class="btn btn-outline-secondary" type="button" id="qualifyBtn">승인처리</button>
		...
		$(function() {
			...
			$('#inputGroupFileAddon05').click(function(){
				...
				if (confirm("다운로드 하시겠습니까? ")){
					location.href='<c:url value="/apply/ghDownload?fileloca=${AppGhDet.ghTotalFile}" />'
			...
			$('#qualifyBtn').click(function(){
				...
				if (confirm("승인처리 하시겠습니까?")){
					...
					if (qualify === 'Y'){
						...
						$.ajax({
							type:'POST',
							url : '<c:url value="/apply/ghChangeQualify" />',
							data : id,
							contentType : 'application/json',
							
							success : function(data){
								if(data === '${AppGhDet.joinId}'){
									alert("승인 처리 완료되었습니다.");
										// 다시 똑같은 디테일로 보내기 
										// 그럴러면 아이디를 보내줘야함 
	/* 									$('#qualify').val('Y'); 
	 */									
										location.href='<c:url value="/apply/ghDetail/${AppGhDet.joinId}" />'
			...
			if (confirm("부적격 처리 하시겠습니까?")){
				...
				if (unqualify === 'Y'){
					...
					$.ajax({
						type:'POST',
						url : '<c:url value="/apply/ghChangeUnQualify" />',
						data : id,
						contentType : 'application/json',
